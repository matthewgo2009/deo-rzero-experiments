#!/bin/bash
# Launch baseline_klfix + conf filter combo in tmux.
# Same vllm setup as canonical (base + 3 solver DP); solvers start with base.
...
echo "Restarting vllm endpoints (need base initially for iter 1)..."
...
sudo rm -rf "$HF_CACHE"/datasets/yuyang322___deo_baseline_klfix_conf* \
            "$HF_CACHE"/datasets/_*yuyang322___deo_baseline_klfix_conf*.lock 2>/dev/null || true

tmux kill-session -t "$SESSION" 2>/dev/null || true
tmux new-session -d -s "$SESSION" -x 220 -y 50

tmux send-keys -t "$SESSION" "sudo docker run --rm \
    --name deo_bkfc_runner \
    --gpus '\"device=2,3\"' \
    ...
    rzero:latest \
    baseline_klfix_conf_main.py 2>&1 | tee ${DEO_STORAGE}/logs/run_baseline_klfix_conf_\$(date +%Y%m%d_%H%M%S).log" Enter

echo ""
echo "baseline_klfix + conf filter running in tmux session '${SESSION}'"
echo "  tmux attach -t ${SESSION}"
echo "Results: ${DEO_STORAGE}/results_summary_baseline_klfix_conf.json"
