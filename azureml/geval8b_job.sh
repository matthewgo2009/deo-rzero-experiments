#!/bin/bash
# 8B general-domain eval: base + DEO(cool_loquat) + baseline(mighty_chicken) + R-Zero.
set -u
...
MODELS="base8b=Qwen/Qwen3-8B-Base"
for i in 1 2 3 4 5; do
  c=$(pick_ckpt "$DEO_IN/DEO/models" "deo_fixedbeta_2000" "$i")
  [ -n "$c" ] && MODELS="$MODELS;deo8b_i$i=$c"
  b=$(pick_ckpt "$BL_IN/DEO/models" "deo_baseline_drift" "$i")
  [ -n "$b" ] && MODELS="$MODELS;baseline8b_i$i=$b"
  r=$(pick_ckpt "$RZ_IN/R-Zero_run/models" "qwen3-8b-base-rzero" "$i")
  [ -n "$r" ] && MODELS="$MODELS;rzero8b_i$i=$r"
done
echo "[geval8b] GEVAL_MODELS=$MODELS"
