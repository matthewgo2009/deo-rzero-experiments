#!/bin/bash
# OctoThinker-8B general-domain eval: base + DEO + baseline + R-Zero, all iters.
set -u
...
MODELS="octo8b_base=OctoThinker/OctoThinker-8B-Hybrid-Base"
for i in 1 2 3 4 5; do
  c=$(pick_ckpt "$DEO_IN/DEO/models" "deo_octo8b_fb" "$i")
  [ -n "$c" ] && MODELS="$MODELS;octo8b_deo_i$i=$c"
  b=$(pick_ckpt "$BL_IN/DEO/models" "deo_baseline_drift" "$i")
  [ -n "$b" ] && MODELS="$MODELS;octo8b_baseline_i$i=$b"
  r=$(pick_ckpt "$RZ_IN/R-Zero_run/models" "octothinker-8b-rzero" "$i")
  [ -n "$r" ] && MODELS="$MODELS;octo8b_rzero_i$i=$r"
done
echo "[geval-octo8b] GEVAL_MODELS=$MODELS"
